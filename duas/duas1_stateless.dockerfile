...
ENV DU_NODE ${DU_NODE:-docker_node1}
ENV DU_HOST ${DU_HOST:-duas1}
ENV DU_IOPORT ${DU_IOPORT:-10600}
ENV DU_LASTPORT ${DU_LASTPORT:-10617}
...
COPY ./wait-for-it.sh .
COPY ./uxcmd .
RUN chmod +x duas1_start.sh wait-for-it.sh uxcmd
 
ENTRYPOINT ["/bin/bash", "duas1_start.sh"] 
